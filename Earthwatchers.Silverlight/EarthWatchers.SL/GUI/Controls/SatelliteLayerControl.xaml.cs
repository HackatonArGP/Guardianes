...
namespace EarthWatchers.SL.GUI.Controls
{
    public partial class SatelliteLayerControl
    {
        ...
        private void UpdateInfo()
        {
            ...
            var dateTime = satelliteLayer.Published;
            dateTime.Value.ToLocalTime();

            if (satelliteLayer.UrlMetadata != null)
                txtInfo.NavigateUri = new Uri(satelliteLayer.UrlMetadata);

            txtPublished.Text = string.Format("{0:MM-dd-yyyy}", dateTime);
            ...
            set
            {
                ...
                if (value)
                {
                   // var tms = new DfaTileSource("http://geodan.blob.core.windows.net/landsat/test/LE71200602008218EDC00");
                    ...
                    Current.Instance.LayerHelper.AddLayer(new TileLayer(tms) { LayerName = satelliteLayer.Name }, false);
                ...
                {
                    Current.Instance.LayerHelper.RemoveLayer(satelliteLayer.Name);
                }
            }
        }
